...
def resolve_attn(requested: str) -> str:
    if requested != "auto":
        return requested
    print("⚠️  Using sdpa (flash_attn not compatible with server glibc)")
    return "sdpa"
...
def build_dataset(tokenizer, sanity: bool = False):
    ...
    if sanity:
        ...
        numina  = load_numinamath(tokenizer,  n_samples=100000)
        meta    = load_metamathqa(tokenizer,  n_samples=50000)
...
def build_training_args(args, sanity: bool, output_dir: Path):
    ...
    # Sanity: only 200 steps, reduced memory settings
    max_steps = 200 if sanity else -1
    save_steps = 9999 if sanity else 500
    logging_steps = 10 if sanity else 50

    # Reduce memory for sanity run — 4096 seq len is enough to verify pipeline
    batch_size = 1 if sanity else args.batch
    seq_len    = 4096 if sanity else args.max_seq_len
    grad_accum = 16 if sanity else args.grad_accum   # keep effective batch ~32
    ...
    print(f"\nEffective batch size: {batch_size} × {grad_accum} × {n_gpus} GPUs = {eff_batch}")

    # trl 0.29.0 — all these are valid SFTConfig fields, no filtering needed
    return SFTConfig(
        ...
        # SFT-specific (trl 0.29.0 — live in SFTConfig)
        max_length=seq_len,
        dataset_text_field="text",
        # Packing disabled — causes memory bloat without flash attention
        # Re-enable once flash_attention_2/3 is confirmed working
        packing=False,
...
def main():
    ...
    # ── SFTTrainer ──
    # trl 0.29.0: processing_class (not tokenizer), dataset_text_field +
    # max_seq_length + packing all live in SFTConfig (not SFTTrainer)
    trainer = SFTTrainer(
        model=model,
        args=train_args,
